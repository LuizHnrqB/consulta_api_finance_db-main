...
def consutar_dados_financeiros() -> Cotacao:
...
def carregar_cotacao_hoje() -> Cotacao:
    ...
    registro_cotacao_hoje = cotacaoDAO.buscar_cotacao()

    if registro_cotacao_hoje is None:
        cotacao = consutar_dados_financeiros()
        salvar_cotacao(cotacao)
        return cotacao

    else:
        registro_cotacao_hoje = cotacaoDAO.buscar_cotacao_hoje()
        return Cotacao(registro_cotacao_hoje[0], registro_cotacao_hoje[1], registro_cotacao_hoje[2], registro_cotacao_hoje[3])
...
def mostrar_menu():
    print(f'Data: {data_hora_hoje}')
    print(f'Cotação do Dolar: ${ cotacao_hoje.dolar }')
    print(f'Cotação do Euro: €{ cotacao_hoje.euro }')
    print(f'Digite o valor em Reais ou digite 0 para terminar: ')

    valores_reais = float(input('R$: '))

    if valores_reais > 0:
        real_em_dolar = valores_reais / cotacao_hoje.dolar
        real_em_euro = valores_reais / cotacao_hoje.euro
        print(f'\n R$ {valores_reais} = $ {real_em_dolar}')
        print(f'\n R$ {valores_reais} = € {real_em_euro}')
        print('\n')
